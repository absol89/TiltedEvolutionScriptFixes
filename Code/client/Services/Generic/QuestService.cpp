                                     ...
                                     const EventDispatcher<TESQuestStartStopEvent>*)
{
    ...
    // PR #846: prevent non-leader teammates from forwarding QuestStop for an already-stopped
    // (incomplete) quest, which would otherwise re-sync a stop for quests still in progress on others.
    if (!isLeader && pQuest->IsStopped())
